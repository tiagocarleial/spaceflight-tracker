...
// Tiangong NORAD ID: 48274 (Tianhe core module)
const TIANGONG_API_BASE = 'https://api.wheretheiss.at/v1/satellites/48274';
...
export async function getTiangongPosition(): Promise<ISSPosition | null> {
  try {
    const response = await fetch(TIANGONG_API_BASE, {
...
export async function getTaikonauts(): Promise<Astronaut[]> {
  try {
    ...
    return data.people.filter(person => person.craft === 'Tiangong');
  } catch (error) {
    console.error('Error fetching taikonauts:', error);
    // Return default crew if API fails (Tiangong typically has 3 crew members)
    return [];
  }
}
...
export async function getTiangongFuturePositions(hoursAhead: number = 2): Promise<[number, number][]> {
  try {
    ...
    const response = await fetch(
      `${TIANGONG_API_BASE}/positions?timestamps=${timestampsString}&units=kilometers`,
      {
        cache: 'no-store',
      }
    );
